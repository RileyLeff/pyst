...
impl Contexts {
    ...
    pub fn evaluate_script(&self, context_name: &str, script_name: &str) -> ContextEvaluation {
        ...
        for (index, pattern) in context.enabled.iter().enumerate() {
            let (is_negation, clean_pattern) = if pattern.starts_with('!') {
                (true, &pattern[1..])
            } else {
        ...
        ContextEvaluation {
            ...
            matched_rules,
            final_rule,
            all_rules: context.enabled.iter().enumerate().map(|(index, pattern)| {
                let (is_negation, clean_pattern) = if pattern.starts_with('!') {
                    (true, &pattern[1..])
                } else {
                    (false, pattern.as_str())
