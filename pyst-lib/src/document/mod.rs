...
impl Documenter {
    ...
    pub async fn document(&self, script_path: &str, write: bool, check: bool) -> Result<String> {
        ...
        let new_description = self
            .generate_description(&script_content, &introspection)
            .await?;
        ...
        self.interactive_documentation(&script_path, &introspection, &new_description)
            .await
    }
    ...
    async fn generate_description(
        ...
        // Prepare script data
        let functions: Vec<FunctionInfo> = introspection
            .metadata
            .functions
            .iter()
            .map(|f| FunctionInfo {
        ...
        let dependencies: Vec<String> = introspection
            .metadata
            .dependencies
            .iter()
            .map(|d| d.name.clone())
        ...
        let current_description = introspection
            .metadata
            .description
            .clone()
            .unwrap_or_default();
    ...
    async fn call_documenter_helper(&self, request: &DocumentationRequest) -> Result<String> {
        ...
        response
            .description
            .ok_or_else(|| anyhow!("No description generated"))
    }
    ...
    async fn check_documentation(&self, introspection: &IntrospectionResult) -> Result<String> {
        match &introspection.metadata.description {
            ...
            _ => Err(anyhow!("❌ No documentation found")),
        }
    }

    async fn interactive_documentation(
        &self,
        script_path: &Path,
        introspection: &IntrospectionResult,
        new_description: &str,
        ...
        match input.trim().to_lowercase().as_str() {
            ...
            _ => Ok("❌ Documentation update cancelled".to_string()),
        }
    }
