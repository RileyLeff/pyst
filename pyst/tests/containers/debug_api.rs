use testcontainers::{runners::AsyncRunner, GenericImage, ImageExt, core::ExecCommand};

#[tokio::test]
async fn debug_container_api() {
    // Use a container that stays running with a long-running process
    let image = GenericImage::new("python", "3.11-slim")
        .with_cmd(vec!["sleep", "infinity"]); // Keep container running
    ...
    match result {
        Ok(mut exec_result) => {
            ...
            let stdout = exec_result.stdout_to_vec().await.expect("Failed to get stdout");
            let stdout_str = String::from_utf8_lossy(&stdout);
            println!("Whoami output: {}", stdout_str.trim());
            
            if let Some(exit_code) = exec_result.exit_code().await.expect("Failed to get exit code") {
    ...
    match result {
        Ok(mut exec_result) => {
            ...
            let stdout = exec_result.stdout_to_vec().await.expect("Failed to get stdout");
            let stderr = exec_result.stderr_to_vec().await.expect("Failed to get stderr");
    ...
    match ls_result {
        Ok(mut exec_result) => {
            ...
            let stdout = exec_result.stdout_to_vec().await.expect("Failed to get stdout");
            let stdout_str = String::from_utf8_lossy(&stdout);
            println!("LS output (first 200 chars): {}", 
