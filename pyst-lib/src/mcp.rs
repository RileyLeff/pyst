...
impl McpServer {
    ...
    async fn handle_tools_list(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        let tools = vec![
            ...
            McpTool {
                ...
                description: "Explain why a script is enabled or disabled in the current context"
                    .to_string(),
                input_schema: serde_json::json!({
                    "type": "object",
                    "properties": {
    ...
    async fn handle_resources_list(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        let resources = vec![
            McpResource {
                ...
                description: "Current pyst configuration including contexts and settings"
                    .to_string(),
            ...
            McpResource {
                ...
                description: "Information about the current project and discovered scripts"
                    .to_string(),
                mime_type: "application/json".to_string(),
            },
        ];
    ...
    async fn handle_resources_read(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        ...
        let content = match uri.unwrap() {
            "pyst://config" => serde_json::to_string_pretty(&self.config)
                .unwrap_or_else(|_| "Error serializing config".to_string()),
            "pyst://project-info" => self
                .get_project_info()
                .await
    ...
    async fn tool_list_scripts(&self, arguments: Option<&serde_json::Value>) -> Result<String> {
        ...
        for (script, introspection) in scripts.iter().zip(introspection_results.iter()) {
            let enabled = self
                .config
                .contexts
                .is_script_enabled(context, &script.name);
    ...
    async fn tool_run_script(&self, arguments: Option<&serde_json::Value>) -> Result<String> {
        ...
        let pyst_exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("pyst"));
    ...
    async fn tool_explain_script(&self, arguments: Option<&serde_json::Value>) -> Result<String> {
        ...
        if evaluation.matched_rules.len() > 1 {
            output.push("".to_string());
            output.push(
                "Note: Multiple rules matched. The last matching rule takes precedence."
                    .to_string(),
            );
        }
