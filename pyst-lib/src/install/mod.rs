...
impl Installer {
    ...
    pub async fn install(&self, source: &str, name: Option<&str>) -> Result<Vec<String>> {
        ...
        match install_source {
            InstallSource::GitHubRepo { owner, repo, path } => {
                self.install_from_github(&owner, &repo, path.as_deref(), name)
                    .await
            }
            InstallSource::GitHubGist { gist_id, file } => {
                self.install_from_gist(&gist_id, file.as_deref(), name)
                    .await
            }
            InstallSource::RawUrl { url } => self.install_from_url(&url, name).await,
        }
    }
        ...
        if let Some(specific_file) = file_path {
            ...
            if file_path.exists() && file_path.extension().and_then(|s| s.to_str()) == Some("py") {
                let script_name = name
                    .unwrap_or(&file_path.file_stem().unwrap().to_string_lossy())
                    .to_string();
    ...
    async fn install_from_gist(
        ...
        for (filename, file_data) in files {
            ...
            let script_name = name
                .unwrap_or(filename.strip_suffix(".py").unwrap_or(filename))
                .to_string();
    ...
    async fn install_from_url(&self, url: &str, name: Option<&str>) -> Result<Vec<String>> {
        ...
        let script_name = if let Some(name) = name {
            ...
            let filename = url_path.split('/').next_back().unwrap_or("script");
    ...
    async fn install_script_file(
        ...
        manifest
            .scripts
            .insert(script_name.to_string(), installed_script);
